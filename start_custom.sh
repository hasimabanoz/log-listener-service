...
echo "---- Start ZooKeeper & Kafka----"
# start ZooKeeper
/opt/kafka/kafka_2.12-2.2.1/bin/zookeeper-server-start.sh /opt/kafka/kafka_2.12-2.2.1/config/zookeeper.properties &
echo "sleep 10s for zookeeper"
sleep 10

# start Kafka
/opt/kafka/kafka_2.12-2.2.1/bin/kafka-server-start.sh /opt/kafka/kafka_2.12-2.2.1/config/server.properties &
echo "sleep 10s for kafka"
sleep 10

# create test topic
/opt/kafka/kafka_2.12-2.2.1/bin/kafka-topics.sh --create --bootstrap-server localhost:9092 --replication-factor 1 --partitions 1 --topic test
...
if [ "$LOGSTASH_START" -ne "1" ]; then
  ...
  echo "Logstash will be started manually"
else
...
if [ "$KIBANA_START" -ne "1" ]; then
  ...
  echo "Kibana node options: " + $NODE_OPTIONS
